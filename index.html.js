...
.btn.gold:hover{
  ...
  transform:translateY(-1px);
    ...
    /* ===== FOOTER ===== */
    /* Ödeme / Güvenlik bantı (legal üstünde) */
.payments{
  max-width:1200px;
  margin:0 auto 12px;      /* footer’dan önce hafif boşluk */
  padding:8px 12px;
}
.payments img{
  width:100%;
...
@media (min-width:901px){
  ...
  position:relative;
  width:100%;
  aspect-ratio:1/1; /* kare oran: tüm resimler eşit boyda görünür */
  background:#f9f9f9;
  overflow:hidden;
  display:flex;
  align-items:center;
...
.card .ph img{
  width:100%;
  height:100%;
  object-fit:cover; /* taşarsa kes, her zaman aynı boyutta */
}
...
<body>
  ...
  <div class="hero">
    ...
    <p id="heroDesc" data-i18n="hero.desc">
      Üreten kadınlardan, ev yapımı lezzetlerden, el işi tasarımlardan alışveriş yapın.
      Konumunuza göre arayın, kategorilerden keşfedin.
    </p>
    <div class="cta-row">
...
<!-- ⬇️ Bu scripti YUKARIDAKİ section'ın hemen altına ekle -->
<script>
  const heroMessages = {
    tr: [
      "PayTR ile taksitli sat, peşin al — paran Üreteneller.com güvencesinde!",
      "El emeğini dünyaya ulaştır, kazancını güvenle al.",
      "Güvenli satışın adresi: Üreteneller.com & PayTR!",
      "Evde üret, Türkiye’ye sat — ödemeni anında al!",
      "PayTR koruması ile her satışta için rahat olsun.",
      "Üreteneller.com’da her satış, %100 güvenle gerçekleşir.",
      "Kadın üreticilerin emeği, PayTR güvencesiyle alıcıda!",
      "Taksitle sat, paranı peşin al — sadece Üreteneller.com’da!",
      "Üreteneller.com & PayTR: Emeğini değerlendirmenin en güvenli yolu.",
      "El emeği kazanca dönüşüyor, paran güvenle hesabına geçiyor.",
      "Her satışında PayTR güvencesi, her alışverişinde huzur!",
      "Üreten kadınlar için güçlü bir ticaret ağı!",
      "Kazancını koru, satışını güvenle tamamla.",
      "Üreteneller.com — güvenli ödeme, mutlu üretici!",
      "Satış yap, kazancın anında senin olsun — PayTR farkıyla!"
    ],
    en: [
      "Sell with installments, get paid instantly — your money is safe with PayTR & Ureteneller.com.",
      "Turn your handmade work into income safely.",
      "Secure sales powered by Ureteneller.com and PayTR!",
      "Create, sell, and get paid instantly — all under PayTR protection.",
      ...
      "Installment sales, instant payments — stress free!",
      "Your craft, your income, your safety — with PayTR.",
      ...
      "Trade with confidence — PayTR keeps you safe."
    ],
    de: [
      "Verkaufe mit Ratenzahlung, erhalte dein Geld sofort — sicher mit Üreteneller.com & PayTR.",
      "Handgemachtes verkaufen, Einkommen sichern!",
      "Jeder Verkauf ist durch PayTR geschützt.",
      "Vertraue auf sichere Zahlungen mit Üreteneller.com.",
      "Zahle in Raten, erhalte dein Geld sofort!",
      "Sicherheit bei jedem Verkauf, Dank PayTR.",
      "Deine Arbeit, dein Gewinn, deine Sicherheit.",
      "Sichere Plattform für kreative Verkäuferinnen!",
      "Verkaufe weltweit — PayTR schützt dein Einkommen.",
      "Zuverlässige Zahlungen, zufriedene Verkäufer.",
      "PayTR-Schutz für Käufer und Verkäufer.",
      "Sicherheit und Vertrauen bei jedem Geschäft.",
      "Schnelle Auszahlungen, garantierte Sicherheit.",
      "Üreteneller.com & PayTR — das Vertrauen zählt.",
      "Dein Handwerk verdient sicheren Erfolg."
    ],
    ar: [
      "بع بالتقسيط وتسلم المبلغ فورًا — أموالك بأمان مع Üreteneller.com و PayTR.",
      "منتجات يدوية بأمان، أرباح فورية!",
      "مدفوعات آمنة مضمونة من PayTR.",
      "اصنع، بِع، واستلم أموالك بثقة.",
      "حماية PayTR تضمن حقوقك دائمًا.",
      "ثقة وأمان في كل عملية بيع وشراء.",
      "منصة Üreteneller.com — المكان الآمن للإبداع والربح.",
      "بيعك مضمون، وربحك محمي مع PayTR.",
      "نظام دفع موثوق لكل المبدعين.",
      "أمانك أولويتنا في Üreteneller.com.",
      "تسوق وبيع بثقة تامة مع PayTR.",
      "حلول دفع مضمونة وسريعة.",
      "اربح بأمان واطمئنان.",
      "كل عملية بيع تحميها PayTR.",
      "المنتج اليدوي + الأمان = نجاح مضمون!"
    ]
  };
  ...
  window.addEventListener('ue:lang', rotateHeroText);
    ...
    <section class="payments" aria-label="Ödeme ve güvenlik">
...
document.getElementById('loginBtn')?.addEventListener('click', async () => {
  ...
  try {
    ...
    redirectHome(); // yalnız doğrulanmışsa
  } catch (err) {
    let msgTr;
    switch (err.code) {
      ...
      case 'auth/wrong-password':
        msgTr = 'Şifre yanlış. Lütfen tekrar deneyin.';
        break;
...
document.getElementById('forgotBtn')?.addEventListener('click', async () => {
  ...
  if (!email) {
    alert(currentLang === 'en'
      ...
      : 'Şifre sıfırlamak için e-posta girin.');
    return;
  }
  try {
    ...
    alert(currentLang === 'en'
      ...
      : 'Şifre sıfırlama e-postası gönderildi. Gelen kutusu ve SPAM klasörünü kontrol edin.');
  } catch (err) {
    let msgTr;
    switch (err.code) {
      case 'auth/user-not-found':
...
document.getElementById('browseBtn')?.addEventListener('click', () => {
  ...
  let listLang = (window.UE && window.UE.getLang && window.UE.getLang()) || localStorage.getItem('ue_lang') || 'tr';

  // dil değişince listeyi yeniden çiz
window.addEventListener('ue:lang', (e)=>{
  listLang = e.detail || 'tr';
...
async function fetchSeller(uid){
  if (!uid) return null;
  try {
    const { getDoc, doc } = await import('./firebase/firebase-firestore.js');
    // Ana koleksiyon
  ...
  const txt = (I18N[listLang]['list.range'] || '{start}-{end} / {total}')
    .replace('{start}', start)
    .replace('{end}', end)
    .replace('{total}', total);
...
if (self.__fbReady) {
  ...
  document.addEventListener('fb-ready', loadListings, { once: true });
    ...
    window.addEventListener('storage', (e)=>{
      if(e.key==='ue_lang'){
        listLang = localStorage.getItem('ue_lang') || 'tr';
        render();
      }
    });
  </script>
